...
// O(MAXN+log(m)) precompute
// O(1) ncr compute


long long MAXN = 1e6+7;
vector<long long>factorial(MAXN+7);
vector<long long>inverse_factorial(MAXN+7);
long long m = 1e9+7;
...
void precalcFacts(){
    ...
    inverse_factorial[MAXN-1] = modInverse(factorial[MAXN-1], m);

    for (int i = MAXN-2; i >= 0; i--) {
        inverse_factorial[i] = (inverse_factorial[i + 1] * (i+1)) % m;
    }
}

long long nCr_reg(int n, int r) {
    if (r < 0 || r > n) return 0;
    return factorial[n] * inverse_factorial[r] % m * inverse_factorial[n - r] % m;
}
